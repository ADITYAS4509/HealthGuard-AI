...
/* ============================================================
   NAV LINK SMOOTH SCROLL + SECTION REVEAL
   Ensures all nav anchor links work even when sections are hidden.
   ============================================================ */
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('a[href^="#"]').forEach(link => {
            link.addEventListener('click', function (e) {
                ...
                // Reveal section if CSS or inline hidden
                ...
                // Hospitals nav: trigger a search if not yet loaded
                if (targetId === 'hospitals-section') {
                    setTimeout(() => {
                        if (typeof triggerHospitalSearch === 'function') triggerHospitalSearch();
                    }, 350);
                }

                // Results nav: show placeholder if no analysis run yet
                if (targetId === 'results-section') {
                    const condName = document.getElementById('conditionName');
                    if (condName && (!condName.textContent.trim() || condName.textContent.trim() === '—')) {
                        condName.textContent = 'Run symptom analysis first to see results.';
                    }
                }
